...
Note: How to add special tokens to Qwen2.5:
  download our model checkpoint with special tokens added: https://huggingface.co/NeuroVLA/Qwen2.5-VL-3B-Instruct-Action
  or /NeuroVLA/model/modules/vlm/tools/add_qwen_special_tokens/README.md (adpat a little code)
...
from NeuroVLA.training.trainer_utils import initialize_overwatch
...
from NeuroVLA.model.modules.action_model.MLP_ActionHeader import get_action_model
...
class Qwenvl_OFT(baseframework):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.action_token = (
            "🔍"  # TODO also can add spacail token to Qwen, but too complex
        )
    ...
    def predict_action(
        ...
        batch_images: List[
            List[Image.Image]
        ],  # Batch of PIL Image list as [view1, view2]
    ...
    def _gather_action_token_embeddings(
        ...
        if isinstance(action_token_id, (list, tuple, set)):
            id_list = torch.tensor(
                list(action_token_id), device=device, dtype=input_ids.dtype
            )
        ...
        if (counts < self.chunk_len).any():
            insufficient = (
                (counts < self.chunk_len).nonzero(as_tuple=False).flatten().tolist()
            )
            raise RuntimeError(
        ...
        topk_pos = masked_pos.topk(
            k=self.chunk_len, dim=-1
        ).values  # [B, chunk_len] 未排序
        # 时间顺序排序
        selected_pos = topk_pos.sort(dim=-1).values  # [B, chunk_len]

        # Gather
        expanded_index = selected_pos.unsqueeze(-1).expand(
            -1, -1, H
        )  # [B, chunk_len, H]
        action_queries = last_hidden.gather(
            dim=1, index=expanded_index
        )  # [B, chunk_len, H]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_yaml",
        type=str,
        default="./NeuroVLA/config/training/internvla_cotrain_oxe.yaml",
    ...
    predict_output = model.predict_action(
        batch_images=[batch[0]["image"]], instructions=[batch[0]["lang"]]
    )
    ...
    # from NeuroVLA.dataloader.lerobot_datasets import get_vla_dataset, collate_fn
